...
| 1 npm | **Plumbing landed.** Unscoped `threejs-doctor` placeholder + scoped `publishConfig` + `publish.yml` (OIDC provenance + optional `NPM_TOKEN`). Packages are **not** on the registry until Kunal creates `@threejs-doctor` and runs Publish once. See [`docs/publish-checklist.md`](../../publish-checklist.md). |

### Residual (not this PR)

- **npm unpublished until Kunal creates the org + `NPM_TOKEN` (then Trusted Publisher).** Agent does not publish without a token. After first publish: `npm i @threejs-doctor/runtime` / `npx threejs-doctor`.
...
- **npm publish** waits on Kunal’s one-time org + `NPM_TOKEN` (then Trusted Publisher). Plumbing: [`docs/publish-checklist.md`](../../publish-checklist.md).
