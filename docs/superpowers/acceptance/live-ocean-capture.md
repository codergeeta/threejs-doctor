...
CLI/runtime for static scan is **`npx threejs-doctor@0.1.1`** (on npm). **0.1.2 is pending publish.** The live-attach IIFE is still this repo file — it is not published. Scan/ci is **not** a substitute for this phone capture.
...
Bundled games (tanks / catapult): **arm capture, then paste the IIFE.** Either paste [`examples/host-shim/capture.js`](../../../examples/host-shim/README.md), wait one frame, then paste `attach.iife.js`; or set `window.__THREEJS_DOCTOR_ATTACH__ = { autoRun: false }`, paste the IIFE, call `ThreejsDoctorLiveAttach.installRendererRenderCapture()`, wait one frame, then `attachQualityLadder`. `installRendererRenderCapture` does not exist until the IIFE has run. Default paste does **not** deep-walk the page (live moonbase froze on a 50k `isWebGLRenderer` BFS). For bundled hosts set `window.__THREEJS_DOCTOR_ATTACH__ = { deepWalk: true }` to opt into a bounded walk (5000 nodes / depth 8 / 80ms). **Re-test those hosts on the box.** If the renderer is fully closed over, pass `{ scene, camera, renderer }` explicitly.
